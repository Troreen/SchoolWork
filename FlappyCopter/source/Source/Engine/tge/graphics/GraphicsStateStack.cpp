...
bool GraphicsStateStack::Init()
{
	...
	
	{
		...
		HRESULT result = DirectX::CreateDDSTextureFromFile(
			DX11::Device,
			string_cast<std::wstring>(Settings::ResolveEngineAssetPath("Textures/whiteCubeMap.dds")).c_str(),
			nullptr, &myDefaultCubemap);
		assert(!FAILED(result));

		ID3D11Resource* resource;
		myDefaultCubemap->GetResource(&resource);
		ID3D11Texture2D* cubeTexture = reinterpret_cast<ID3D11Texture2D*>(resource);
		D3D11_TEXTURE2D_DESC cubeDescription;
		cubeTexture->GetDesc(&cubeDescription);

		myDefaultCubemapMipCount = cubeDescription.MipLevels;
	}

	{
		...
		HRESULT result = DirectX::CreateDDSTextureFromFile(
			DX11::Device,
			string_cast<std::wstring>(Settings::ResolveEngineAssetPath("Textures/horizonCubeMap.dds")).c_str(),
			nullptr, &myDefaultHorizonCubemap);
		assert(!FAILED(result));

		ID3D11Resource* resource;
		myDefaultHorizonCubemap->GetResource(&resource);
		ID3D11Texture2D* cubeTexture = reinterpret_cast<ID3D11Texture2D*>(resource);
		D3D11_TEXTURE2D_DESC cubeDescription;
		cubeTexture->GetDesc(&cubeDescription);

		myDefaultHorizonCubemapMipCount = cubeDescription.MipLevels;
	}
...
void GraphicsStateStack::UpdateGpuStates(bool fullReset)
{
	...
	if (fullReset || state.lightDataVersion != myGpuRenderState.lightDataVersion)
	{
		...
		*dataPtrLights = {};
		...
		switch (ambientLight.GetAmbientLightType())
		{
		case AmbientLightType::Custom:
			dataPtrLights->myNumEnvMapMipLevels = ambientLight.GetNumMips();
			break;
		case AmbientLightType::Uniform:
			dataPtrLights->myNumEnvMapMipLevels = myDefaultCubemapMipCount;
			break;
		case AmbientLightType::UniformAboveHorizon:
			dataPtrLights->myNumEnvMapMipLevels = myDefaultHorizonCubemapMipCount;
			break;
		}
		...
		case AmbientLightType::Custom:
			...
			break;
		case AmbientLightType::Uniform:
			DX11::Context->PSSetShaderResources(0, 1, myDefaultCubemap.GetAddressOf());
			break;
		case AmbientLightType::UniformAboveHorizon:
			DX11::Context->PSSetShaderResources(0, 1, myDefaultHorizonCubemap.GetAddressOf());
			break;
		}
