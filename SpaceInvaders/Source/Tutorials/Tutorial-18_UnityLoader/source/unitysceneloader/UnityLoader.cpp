...
static void TraverseObject(json object, RenderData& someRenderData) {
	...
	if (type == "camera") {
		...
		camera.GetTransform().SetPosition(position * 100.0f);
		// rotation?
	...
	{
		std::shared_ptr<ModelInstance> obj = std::make_shared<ModelInstance>(modelFactory.GetModelInstance(object["path"]));

		Matrix4x4f t;
		t.SetPosition(position * 100.0f);
		t.Scale(scale);
		t.Rotate(rotation);
		obj->SetTransform(t);
	...
	else if (type == "sprite") 
	{
		...
		std::string assetpath = (std::string)object["path"];
		...
		Matrix3x3f scale_matrix = Matrix4x4f::CreateFromScale({
			tex_size.x * scale.x,
			tex_size.y * scale.y,
			scale.z
		});
		Matrix3x3f rotation_matrix = Matrix3x3f::CreateFromRotation(rotation);
